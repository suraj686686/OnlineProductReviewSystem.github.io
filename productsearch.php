...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["productname"])) {
  ...
  try {
    // Perform a full-text search query on the 'item' table
    ...
    if (empty($results)) {
      ...
      $rating = new Rating(); // Instantiate the Rating class

      foreach ($results as $item) {
        $average = $rating->getRatingAverage($item["id"]); // Calculate the average rating
